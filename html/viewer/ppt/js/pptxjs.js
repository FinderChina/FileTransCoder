...
(function ($) {
    $.fn.pptxToHtml = function (options) {
        ...
        if (settings.fileData && settings.fileData != "") {
            // 从 Uint8Array 获取的 ArrayBuffer JSZip2不认
            //const arrayBuf = settings.fileData.buffer; // 传入的 fileData 是 Uint8Array
            // 封装成老版JSZip能识别的格式
            const blob = new Blob([settings.fileData]);
            FileReaderJS.setupBlob(blob, {
                readAsDefault: "ArrayBuffer",
                on: {
                    load: function (e, blob) {
                        //console.log(e.target.result);
                        convertToHtml(e.target.result);
                    }
                }
