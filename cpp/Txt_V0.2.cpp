...
public:
    LineNumberTextCtrl(wxWindow* parent, wxWindowID id = wxID_ANY)
        ...
        // 创建行号控件
        lineNumberCtrl = new wxTextCtrl(this, wxID_ANY, "",
                                      wxDefaultPosition, wxDefaultSize,
                                      wxTE_MULTILINE | wxTE_READONLY | wxTE_RICH | 
                                      wxTE_NOHIDESEL | wxTE_DONTWRAP | wxTE_NO_VSCROLL);
        // 创建文本控件
        textCtrl = new wxTextCtrl(this, wxID_ANY, "",
                                 wxDefaultPosition, wxDefaultSize,
                                 wxTE_MULTILINE | wxTE_RICH2 | wxTE_NOHIDESEL | wxTE_DONTWRAP);
        
        // 设置相同的字体和行高
        wxFont font(12, wxFONTFAMILY_MODERN, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL);
        textCtrl->SetFont(font);
        lineNumberCtrl->SetFont(font);
        
        // 设置行号控件的背景色
        lineNumberCtrl->SetBackgroundColour(wxColour(240, 240, 240));
        
        // 设置初始行号宽度
        SetLineNumberWidth(50);
        
        // 添加到布局
        sizer->Add(lineNumberCtrl, 0, wxEXPAND | wxALL, 0);
        sizer->Add(textCtrl, 1, wxEXPAND | wxALL, 0);
        SetSizer(sizer);
        
        // 绑定滚动事件
        textCtrl->Bind(wxEVT_SCROLLWIN_LINEUP, &LineNumberTextCtrl::OnScroll, this);
        textCtrl->Bind(wxEVT_SCROLLWIN_LINEDOWN, &LineNumberTextCtrl::OnScroll, this);
        textCtrl->Bind(wxEVT_SCROLLWIN_PAGEUP, &LineNumberTextCtrl::OnScroll, this);
        textCtrl->Bind(wxEVT_SCROLLWIN_PAGEDOWN, &LineNumberTextCtrl::OnScroll, this);
        textCtrl->Bind(wxEVT_SCROLLWIN_THUMBTRACK, &LineNumberTextCtrl::OnScroll, this);
        textCtrl->Bind(wxEVT_SCROLLWIN_THUMBRELEASE, &LineNumberTextCtrl::OnScroll, this);
        
        // 绑定文本变化事件
        textCtrl->Bind(wxEVT_TEXT, &LineNumberTextCtrl::OnTextChanged, this);
    ...
    void SetLineNumberWidth(int width) {
        lineNumberCtrl->SetMinSize(wxSize(width, -1));
        lineNumberCtrl->SetMaxSize(wxSize(width, -1));
        Layout();
    }
    
private:
    wxTextCtrl* textCtrl;
    wxTextCtrl* lineNumberCtrl;
    ...
    void UpdateLineNumbers() {
        ...
        lineNumberCtrl->SetValue(numbers);
    ...
    void SyncScrollPositions() {
        ...
        lineNumberCtrl->SetScrollPos(wxVERTICAL, scrollPos);
        
        statusBar->SetStatusText(scrollPos, 0); // 第一格显示完整路径
    }
